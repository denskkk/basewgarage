...
window.TaskManager = {
    ...
    init() {
        ...
        console.log('Загружено задач:', this.tasks.length);
    },
    
    // Создание новой задачи
    createTask(taskData) {
        console.log('TaskManager.createTask вызван с данными:', taskData);
        ...
        // Проверяем, существует ли пользователь-исполнитель
        if (!window.usersDatabase[taskData.assignedTo]) {
            console.error('Пользователь-исполнитель не найден');
            return { success: false, message: 'Пользователь-исполнитель не найден' };
        }
        ...
        console.log('Задача сохранена. Всего задач:', this.tasks.length);
    ...
    updateTaskStatus(taskId, status, comment = '') {
        ...
        if (taskIndex === -1) return { success: false, message: 'Задача не найдена' };
        
        const currentUser = window.UserManager.getCurrentUser();
        const oldStatus = this.tasks[taskIndex].status;
        
        this.tasks[taskIndex].status = status;
        this.tasks[taskIndex].updated = new Date().toISOString();
        
        // Добавляем автоматический комментарий об изменении статуса
        this.tasks[taskIndex].comments.push({
            text: `Статус изменен с "${this.getStatusText(oldStatus)}" на "${this.getStatusText(status)}"${comment ? `. Комментарий: ${comment}` : ''}`,
            author: currentUser?.id || 'unknown',
            timestamp: new Date().toISOString(),
            type: 'status_change'
        });
        
        // Если есть дополнительный комментарий, добавляем его отдельно
    ...
    addComment(taskId, comment) {
        ...
        this.tasks[taskIndex].comments.push({
            text: comment,
            author: window.UserManager.getCurrentUser()?.id || 'unknown',
            timestamp: new Date().toISOString(),
            type: 'comment'
        });
    ...
    acceptTask(taskId, comment = '') {
        ...
        // Проверяем, что пользователь может принять эту задачу
        if (task.assignedTo !== currentUser.id) {
            return { success: false, message: 'Вы не можете принять эту задачу' };
        }
        
        if (task.status !== 'new') {
            return { success: false, message: 'Задача уже принята или завершена' };
        }
        
        // Меняем статус на "в работе"
        this.tasks[taskIndex].status = 'in_progress';
        this.tasks[taskIndex].acceptedAt = new Date().toISOString();
        this.tasks[taskIndex].updated = new Date().toISOString();
        
        // Добавляем комментарий о принятии задачи
        this.tasks[taskIndex].comments.push({
            text: `Задача принята в работу${comment ? `. Комментарий: ${comment}` : ''}`,
    ...
    completeTask(taskId, comment = '') {
        ...
        // Проверяем, что пользователь может завершить эту задачу
        if (task.assignedTo !== currentUser.id && !currentUser.canViewAll) {
            return { success: false, message: 'Вы не можете завершить эту задачу' };
        }
        
        if (task.status === 'completed') {
            return { success: false, message: 'Задача уже завершена' };
        }
        ...
        this.tasks[taskIndex].comments.push({
            text: `Задача завершена${comment ? `. Комментарий: ${comment}` : ''}`,
            author: currentUser.id,
            timestamp: new Date().toISOString(),
            type: 'complete'
        });
    ...
    rejectTask(taskId, reason) {
        ...
        // Проверяем, что пользователь может отклонить эту задачу
        if (task.assignedTo !== currentUser.id && !currentUser.canViewAll) {
            return { success: false, message: 'Вы не можете отклонить эту задачу' };
        }
        
        if (task.status === 'completed' || task.status === 'cancelled') {
            return { success: false, message: 'Задача уже завершена или отменена' };
        }
        
        // Меняем статус на "отклонена"
        this.tasks[taskIndex].status = 'rejected';
        this.tasks[taskIndex].rejectedAt = new Date().toISOString();
        this.tasks[taskIndex].updated = new Date().toISOString();
        
        // Добавляем комментарий об отклонении задачи
        this.tasks[taskIndex].comments.push({
            text: `Задача отклонена. Причина: ${reason}`,
            author: currentUser.id,
            timestamp: new Date().toISOString(),
            type: 'reject'
        });
    ...
    saveTasks() {
        console.log('Сохранение задач в localStorage, количество:', this.tasks.length);
        try {
            localStorage.setItem('wgarage-tasks', JSON.stringify(this.tasks));
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM загружен, инициализируем TaskManager...');
